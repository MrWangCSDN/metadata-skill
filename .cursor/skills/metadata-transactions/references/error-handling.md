...
**强制规则**: MCP 返回 null 的字段**禁止写入 XML**，必须从生成列表中移除。

**示例返回**:
```javascript
{
  "客户ID": { id: "custId", type: "...", ... },
  "未知字段": null,  // ← 未贯标 → 强制不写入 XML
  "测试字段": null   // ← 未贯标 → 强制不写入 XML
}
```

**处理结果提示**:
```
⚠️  以下字段未贯标，已从 XML 中强制排除（共 2 个）:
...
💡 如需完整交易，请在 dict-mcp-server 系统中完成字段贯标后重新执行指令
```
